...
| `卡片顯示類型` | `checkbox` | 卡片是否顯示作品類型 |
| `卡片顯示Tag` | `checkbox` | 卡片是否顯示 Tag |
| `作品頁顯示類型` | `checkbox` | 作品頁 Header 是否顯示作品類型 |
| `作品頁顯示日期` | `checkbox` | 作品頁 Header 是否顯示建立日期 |
| `作品頁顯示Tag` | `checkbox` | 作品頁 Header 是否顯示 Tag |
| `published` | `checkbox` | 是否在網站上公開顯示 |

> **顯示邏輯說明：**
> - 封面圖、作品名稱：卡片與作品頁皆固定顯示，不設 checkbox
> - 卡片欄位與作品頁欄位獨立控制，互不影響
> - 前端讀取各 checkbox 值後依此決定是否渲染該欄位
...
**作品卡片顯示欄位：**
- 封面圖：固定顯示（無圖顯示 `--color-bg-surface` 灰底 placeholder）
- 作品名稱：固定顯示
- 作品類型：依 `卡片顯示類型` checkbox（`--color-brand-primary`，Label/Caps 樣式）
- Tag pills：依 `卡片顯示Tag` checkbox
...
#### 作品 Header 顯示欄位
- 作品名稱：固定顯示（`H1`）
- 作品類型：依 `作品頁顯示類型` checkbox
- Tag pills：依 `作品頁顯示Tag` checkbox（Active 樣式，`--color-brand-primary` 背景）
- 建立日期：依 `作品頁顯示日期` checkbox（格式 `YYYY-MM-DD`，`Caption`，`--color-text-disabled`）
